...
)
{
    ...
    if (handle->nplots > 0 && title)
        gnuplot_cmd(handle, "%s %.18e * x + %.18e title \"%s\" with %s",
                  "replot", a, b, title, handle->pstyle) ;
    else if (handle->nplots > 0)
        gnuplot_cmd(handle, "%s %.18e * x + %.18e title \"%s\" with %s",
                  "replot", a, b, "(none)", handle->pstyle) ;
	else if (title)
        gnuplot_cmd(handle, "%s %.18e * x + %.18e title \"%s\" with %s",
                  "plot", a, b, title, handle->pstyle) ;
	else
        gnuplot_cmd(handle, "%s %.18e * x + %.18e title \"%s\" with %s",
                  "plot", a, b, "(none)", handle->pstyle) ;
...
)
{
	...
	if (h->nplots > 0 && title)
        gnuplot_cmd(h, "%s %s title \"%s\" with %s",
                  "replot", equation, title, h->pstyle) ;
	else if (h->nplots > 0)
        gnuplot_cmd(h, "%s %s title \"%s\" with %s",
                  "replot", equation, "(none)", h->pstyle) ;
	else if (title)
        gnuplot_cmd(h, "%s %s title \"%s\" with %s",
                  "plot", equation, title, h->pstyle) ;
	else
        gnuplot_cmd(h, "%s %s title \"%s\" with %s",
                  "plot", equation, "(none)", h->pstyle) ;
...
//	if (handle->nplots > 0 && title)
// K. Loux - we'll need to clean up this whole file at some point...
    gnuplot_cmd(handle, "%s \"%s\" title \"%s\" with %s", "plot", tmp_filename,
                  title, handle->pstyle) ;
    handle->nplots++ ;
    return ;
}
