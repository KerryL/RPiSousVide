...
int main(int, char *[])
{
	...
	LoggingFunctionTakingOStreamArg(logger);

	logger << "Here's a number with trailing zeros:  " << std::setprecision(10) << std::fixed << 1.0 << std::endl;
	logger << "Here's a hex number:  0x" << std::hex << 256 << std::endl;
	logger << "Here's a number in scientific notation:  " << std::scientific << 6548454.8486868 << std::endl;
